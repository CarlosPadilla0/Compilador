            ...
            if (symbolTable.containsKey(variableName)) {
                ...
                if (index + 1 < tokens.size()) {
                    ...
                    if ("BOOLEAN".equals(variableType)) {
                        if (!"INT".equals(expressionType) || !("0".equals(expressionType) || "1".equals(expressionType))) {
                            errores.append("Error: Asignación incorrecta para la variable '")
                                   .append(variableName).append("'. Las asignaciones válidas para BOOLEAN son '0' y '1'.\n");
                        }
                    } 
                    if ("String".equals(variableType)) {
                        if (!"TEXTO".equals(expressionType)) {
                            errores.append("Error: Asignación incorrecta para la variable '")
                                   .append(variableName).append("'. Se esperaba un valor de tipo 'String' (entre comillas dobles), pero se encontró '")
                    ...
                    errores.append("Error: Asignación incompleta para la variable '").append(variableName).append("'.\n");
                }
            } else {
                errores.append("Error: La variable '").append(variableName).append("' no está declarada.\n");
            }
        ...
        switch (token.getTipo()) {
            case NUMERO:
                if (expectingOperand) {
                    if (expressionType == null) {
                        ...
                        errores.append("Error: Tipo incompatible en la expresión. Se esperaba '").append(expressionType).append("'.\n");
                    }
            ...
            case IDENTIFICADOR:
                if (expectingOperand) {
                    if (symbolTable.containsKey(token.getValor())) {
                        ...
                        errores.append("Error: La variable '").append(token.getValor()).append("' no está declarada.\n");
                    }
                } else {
                    errores.append("Error: Operador faltante antes de la variable '").append(token.getValor()).append("'.\n");
                }
                break;
            ...
            case OPERADOR:
                if (!expectingOperand) {
                    ...
                    errores.append("Error: Se esperaba un operando antes de '").append(token.getValor()).append("'.\n");
                }
                break;
